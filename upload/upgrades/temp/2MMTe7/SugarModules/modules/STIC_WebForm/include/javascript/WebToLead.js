...
var add_all_fields = SUGAR.language.get('app_strings',
		'LBL_ADD_ALL_LEAD_FIELDS');
var remove_all_fields = SUGAR.language.get('app_strings',
...
function checkFields(REQUIRED_LEAD_FIELDS, LEAD_SELECT_FIELDS) {
	...
	if (reqFields) {
		...
		return false;
	} else if (grid3.getRecordSet().getLength() == 1
			&& grid4.getRecordSet().getLength() == 1) {
...
function selectFields(indexes, grid) {
	...
	for (var i = 0; i < indexes.length; i++) {
		retStr = retStr
				+ grid.getRow(indexes[i]).childNodes[0].childNodes[0].innerHTML
				+ ',' + '\n';
		retStr = retStr + '\n';
	}
...
function displayAddRemoveDragButtons(Add_All_Fields, Remove_All_Fields) {
	...
	} else if (grid3.getRecordSet().getLength() == 0
			&& grid4.getRecordSet().getLength() == 0) {
	...
	if (addRemove.value == Add_All_Fields && availibleCount > 1) {
		for (var i = 0; i < availibleCount; i++) {
			...
			if (i % 2 == 1 && availibleSet.getRecord(i).getData()[0] != " ") {
				grid4
						.addRow(availibleSet.getRecord(i).getData(),
								((i - 1) / 2));
			}
		}
		for (i = availibleCount - 1; i >= 0; i--) {
			if (grid2.getRecord(i) != null
					&& grid2.getRecord(i).getData()[0] != " ") {
				grid2.deleteRow(i);
			}
		}
	} else if (addRemove.value == Remove_All_Fields) {
		var count = 0;
		if (grid3.getRecordSet().getLength() >= grid4.getRecordSet()
				.getLength()) {
		...
		for (var i = 0; i < count; i++) {
			if (grid3.getRecord(i) != null
					&& grid3.getRecord(i).getData()[0] != " ") {
				grid2.addRow(grid3.getRecord(i).getData(), grid2.getRecordSet()
						.getLength() - 1);
			}
			if (grid4.getRecord(i) != null
					&& grid4.getRecord(i).getData()[0] != " ") {
				grid2.addRow(grid4.getRecord(i).getData(), grid2.getRecordSet()
						.getLength() - 1);
			}
		}
...
/**
 * Retorna un array con los elementos que contengan un atributo con el valor especificado
 * Extraido de http://snipplr.com/view/1853/get-elements-by-attribute/
 * @param oElm				Elemento DOM padre donde se realizará la búsqueda
 * @param strTagName		Tipo de elemento para el que se realizará la búsqueda 
 * @param strAttributeName	Nombre del atributo
 * @param strAttributeValue	Valor a buscar
 * @returns {Array}
 */
...
// Elimina la pestana del Formualario web de los submenús
SUGAR.util.doWhen("typeof($('#subModuleList'))!= 'undefined'",
	    function() {
			var links = getElementsByAttribute(document.getElementById('subModuleList'),'a','module','STIC_WebForm');
