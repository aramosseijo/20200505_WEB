...
//Añadir nuestra validación de JAVASCRIPT que queremos realizar
 ...
 if (!iserror) {
        ...
        if(document.getElementById('numeroidentificacion_c').value != ''){
            if(document.getElementsByName('tipoidentificacion_c')[0].value=='nif'){
                resultado = valida_nif_cif_nie(document.getElementById('numeroidentificacion_c').value);
                if(resultado != 1){
                    iserror = true;
                    alert("NIF no valido");
                }
            }else{ if(document.getElementsByName('tipoidentificacion_c')[0].value=='nie'){
                        resultado = valida_nif_cif_nie(document.getElementById('numeroidentificacion_c').value);
                        if(resultado != 3){
                            iserror = true;
                            alert("NIE no valido");
                        ...
                        if(document.getElementsByName('tipoidentificacion_c')[0].value=='cif'){
                            resultado = valida_nif_cif_nie(document.getElementById('numeroidentificacion_c').value);
                            if(resultado != 2){
                                iserror = true;
                                alert("CIF no valido");
