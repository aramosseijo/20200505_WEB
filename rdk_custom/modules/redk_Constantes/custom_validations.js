...
function validaciones_save(){
   ...
   //Validacion del codigo del cliente: no puede haber 2 clientes iguales
   campo = "codigo";
   valor = getValor(campo);
   xmlhttp.open("GET","index.php?module="+modulo+"&action=custom_validations&field="+campo+"&value="+valor+"&record="+record, false); //false = sincrono; true = asincrono
   xmlhttp.send();
...
function getValorByName(campo){
    ...
    if (vista == "subpanel" || vista == "quick"){ //subformulario
        ...
        //STIC TODO: si no se encuentran en los textos, buscar en los select
    }
