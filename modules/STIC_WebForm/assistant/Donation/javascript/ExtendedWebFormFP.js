...
function adaptaPeriodicidad() {
    ...
    if (v_periodicidad && v_periodicidad != 'puntual' && (v_mediopago == 'tarjeta' || v_mediopago == 'paypal') ) {
...
function adaptaMedioPago() {
  ...
  // Si el medio de pago ha cambiado a paypal, comprueba la periodicidad
	if (v_mediopago == 'paypal' && v_periodicidad && v_periodicidad != 'puntual') {
		if (confirm(redk_Formas_de_pago_LBL_MEDIOPAGO_PUNTUAL)) {
			// Si se quiere continuar, se indica periodicidad puntual
			setSelectValue(o_periodicidad, 'puntual');
		} else {
