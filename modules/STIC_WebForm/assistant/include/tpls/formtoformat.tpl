...
<table width='100%' style='border-top: 1px solid; border-bottom: 1px solid; padding: 10px 6px 12px 10px; background-color: rgb(233, 243, 255); font-size: 12px; background-repeat: repeat-x; background-position: center top;'>
		        		...
		        		<td width="35%" id="td_{$FIELD.name}" style="font-size: 12px; font-weight: normal;{if $FIELD.hidden} display: none{/if}">
							...
							*}
		        			<span {if $FIELD.DEF.type == 'text'} id="ta_replace" {/if} sugar="slot">
								...
								{* Select multiple *}
								{* // 20181214 jch case 2077 Se añaden corchetes específicos para la propiedad name de los campos multiselect *}
	...
	{php}
		for($i = 1; $i <= $_REQUEST["num_attachment"]; $i++){
			$cadena_adjuntos .= '
			<tr>
			<td style="text-align: left; font-size: 12px; font-weight: normal;" width="15%"><span>Documento adjunto '.$i.': </span></td>
			<td style="font-size: 12px; font-weight: normal;" width="35%"><span><input id="documento_'.$i.'" type="file" name="documents[]" /></span></td>
